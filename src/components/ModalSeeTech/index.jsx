...
const ModalSeeTech = ({ user, setUser, handleClose, open, techClicked }) => {
  ...
  const handleChange = () => {
    api
      .put(`/users/techs/${techClicked.id}`, newStatus, {
        headers: {
      ...
      .catch(() =>
        toast.error('Não foi possível alterar a tech, tente novamente')
      )
  }

  const handleDelete = () => {
    api
      .delete(`/users/techs/${techClicked.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      .catch(() =>
        toast.error('Não foi possível excluir a tech, tente novamente')
      )
  }
